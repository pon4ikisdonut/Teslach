...
void c_rage_bot::process_attack(c_user_cmd* user_cmd, vec3_t angle, c_base_player_weapon* active_weapon)
{
	...
	//		input_history->set_player_tick_count(0);
	//		input_history->set_player_tick_count(0);
	//	}
	//}

	//float flWatTickOffset{};
	//float flTemp = active_weapon->m_next_primary_attack_ratio() + modf(active_weapon->m_next_primary_attack_ratio(), &flWatTickOffset);
	//int iPlayerTick = flWatTickOffset + active_weapon->m_next_primary_attack();
	//if (flTemp >= 1.f)
	//	++iPlayerTick;
	//else if (flTemp < 0.f)
	//	--iPlayerTick;
	...
	//for (int i = 0; i < g_ctx->m_user_cmd->pb.input_history_size(); i++) {
	//	CSGOInputHistoryEntryPB* pInputHistoryEntry = g_ctx->m_user_cmd->pb.mutable_input_history(i);
	//	if (!pInputHistoryEntry)
	//		continue;

	//	pInputHistoryEntry->set_player_tick_count(iPlayerTick - 1);
	//	pInputHistoryEntry->set_player_tick_fraction(0.f);
	//}
	...
	double offset_tick{ };
	auto temp = active_weapon->m_next_primary_attack_ratio() + offset_tick;
	auto next_attack = active_weapon->m_next_primary_attack();
	...
	bool f = shot_count % 2;
	std::cout << f << " f value\n";
	std::cout << shoot_tick << " shoot tick\n";
	std::cout << temp << " attack1 history index before\n" << next_attack << "nextattack\n";
	if (user_cmd->pb.attack1_start_history_index() > -1)
		shot_count++;
	for (int i = 0; i < g_ctx->m_user_cmd->pb.input_history_size(); i++) {
		auto input_history = user_cmd->pb.mutable_input_history(i);
		...
		input_history->set_player_tick_count(f ? shoot_tick - 1 : 0);
		input_history->set_player_tick_fraction(0.f);
		std::cout << (f ? shoot_tick - 1 : 0) << " player tick count\n";
	}
	...
	std::cout << (f ? shoot_tick - 1 : 0) << " player tick count\n";
	std::cout << "-----------------------------------------------------\n" << std::endl;
	/*std::cout << "Attack1 historyo index 0" << user_cmd->pb.attack1_start_history_index() << std::endl;
	...
	std::cout << "Attack1 historyo index 1" << user_cmd->pb.attack1_start_history_index() << std::endl;
	user_cmd->pb.set_attack1_start_history_index(0);
	process_backtrack(m_best_target->m_lag_record);
	std::cout << "Attack1 historyo index 2" << user_cmd->pb.attack1_start_history_index() << std::endl;*/
...
void c_rage_bot::on_create_move()
{
	...
	//c_base_player_weapon* active_weapon;
	if (!user_cmd)
		return;
	...
	if (hit_chance >= (is_taser ? 70 : g_cfg->rage_bot.m_hit_chance)) {
		...
		process_attack(user_cmd, best_angle, active_weapon);
	}
