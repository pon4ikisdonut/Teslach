...
public:
	...
	float get_inaccuracy() const noexcept
	{
		// ‘ункци€ в игре Ќ≈ const Ч она пишет в объект, поэтому делаем const_cast
		using fn_t = float(__fastcall*)(c_base_player_weapon*, float*, float*);

		static const fn_t fn = []() -> fn_t {
			const auto addr = g_opcodes->scan(
			...
			return reinterpret_cast<fn_t>(addr);
			}();

		if (!fn)
			return 0.f; // или бросай exception, как хочешь

		float x = 0.f, y = 0.f;
		// const_cast Ч единственный безопасный способ вызвать не-const функцию из const метода
		return fn(const_cast<c_base_player_weapon*>(this), &x, &y);
	}
